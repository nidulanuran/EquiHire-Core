...
const STAGES = [
  ...
  { id: 'scheduled', label: 'Screening', color: 'bg-yellow-500' },
    ...
    <div className="flex gap-4 h-full overflow-x-auto pb-4 scrollbar-hide">
      {STAGES.map((stage) => {
        const stageCandidates = candidates.filter((c) => {
            // "Applied" (pending) are those with 'pending' status and NO evaluation yet (score 0 and no interview/skills scores)
            if (stage.id === 'pending') return c.status === 'pending' && c.score === 0 && !c.interviewScore && !c.skillsScore;
            // "Screening" (scheduled) are those with 'pending' status but HAVE some evaluation data or a score > 0
            if (stage.id === 'scheduled') return c.status === 'pending' && (c.score > 0 || !!c.interviewScore || !!c.skillsScore);
